...
class Handle(StructuralType):
    ...
    Functionally we gain nothing from making this an (abstract) base class for handle implementations,
    so this definition is for type hinting purposes only. You can, but don't have to, inherit from it.
    """
    def open(self) -> None:
        ...
...
class Protocol:
    ...
    A Protocol implements the part of the Transport API that relates to communicating logical
    messages over a physical layer. It is a thing that can:
    ...
    We will need a new Protocol class if we change the way a Trezor device encapsulates its messages.
    """
...
class ProtocolBasedTransport(Transport):
    ...
    Intended as a base class for implementations that proxy their communication operations to a Protocol.
    """
...
class ProtocolV1(Protocol):
    """Protocol version 1. Currently (06/2018) in use on all Trezors. Does not understand sessions."""
...
class ProtocolV2(Protocol):
    """Protocol version 2. Currently (06/2018) not used. Intended to mimic U2F/WebAuthN session handling."""
...
def get_protocol(handle: Handle, want_v2: bool) -> Protocol:
    ...
    Each transport can have a preference for using a particular protocol version. This preference is overridable
    through `TREZOR_PROTOCOL_V1` environment variable, which forces the library to use V1 anyways.

    As of 06/2018, no devices support V2, so we enforce V1 here. It is still possible to set `TREZOR_PROTOCOL_V1=0`
    and thus enable V2 protocol for transports that ask for it (i.e., USB transports for Trezor T).
    """
